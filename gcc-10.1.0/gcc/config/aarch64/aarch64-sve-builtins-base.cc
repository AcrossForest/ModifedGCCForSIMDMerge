...
class svSpSpMemoryBaseStore : public function_base{
  public:
  unsigned int
  call_properties (const function_instance &) const OVERRIDE
  {
...
class svSpSpMemoryBaseLoad : public function_base{
  public:
  unsigned int
  call_properties (const function_instance &) const OVERRIDE
  {
...
#define SVSPSPMODEFUNCTION(CNAME,RTLNAME,BASE) \
class svSpSp_##CNAME##_impl : public BASE{\
  public:\
  rtx \
  expand (function_expander &e) const OVERRIDE \
  { \
    insn_code icode;\
    auto vectorMode = e.vector_mode(0);\
    icode = code_for_##RTLNAME(vectorMode);\
    return e.use_exact_insn (icode);\
  }\
};\
FUNCTION (CNAME, svSpSp_##CNAME##_impl,)


class svindex_compress_impl : public function_base
{
public:
  rtx
  expand (function_expander &e) const OVERRIDE
  {
    insn_code icode;
    gcc_assert (e.vector_mode (0) == E_VNx4SFmode);
    icode = code_for_aarch64_sve_vnx4sf (UNSPEC_INDEX_COMPRESS);
    return e.use_exact_insn (icode);
  }
};

class svring_load_impl : public full_width_access
{
public:
  rtx
  expand (function_expander &e) const OVERRIDE
  {
    insn_code icode;
    gcc_assert (e.vector_mode (0) == E_VNx4SFmode);
    icode = CODE_FOR_aarch64_sve_ring_load_vnx4sf;
    return e.use_exact_insn (icode);
  }
};
...
#define SPSPFIXFUNCTION(CNAME, RTLNAME) \
  FUNCTION(CNAME,fixed_insn_function,(CODE_FOR_##RTLNAME))


SPSPFIXFUNCTION (svSpSpInitBigCmp           ,aarch64_sve_SpSp_init_big_cmp)
SPSPFIXFUNCTION (svSpSpInitBigCmpFromMatRes ,aarch64_sve_SpSp_init_big_cmp_from_mat_res)
SVSPSPMODEFUNCTION (svSpSpKeyCombine        ,aarch64_sve_SpSp_key_combine,function_base)
SPSPFIXFUNCTION (svSpSpMatch                ,aarch64_sve_SpSp_match)
SPSPFIXFUNCTION (svSpSpGetLimit             ,aarch64_sve_SpSp_get_limit)
SVSPSPMODEFUNCTION (svSpSpBFPermute         ,aarch64_sve_SpSp_bf_permute,function_base)
SVSPSPMODEFUNCTION (svSpSpSEPermute         ,aarch64_sve_SpSp_se_permute,function_base)

SPSPFIXFUNCTION (svSpSpPack,aarch64_sve_SpSp_pack)
SPSPFIXFUNCTION (svSpSpIndexCompressInitM1,aarch64_sve_SpSp_idxCompInit_method1)
SPSPFIXFUNCTION (svSpSpGetLength,aarch64_sve_SpSp_getLength)
SPSPFIXFUNCTION (svSpSpIndexCompressInitM2,aarch64_sve_SpSp_idxCompInit_method2)
SPSPFIXFUNCTION (svSpSpIndexCompression,aarch64_sve_SpSp_idxComp)
SPSPFIXFUNCTION (svSpSpIndexMatchM1,aarch64_sve_SpSp_idxMatch_method1)
SPSPFIXFUNCTION (svSpSpIndexMatchM2,aarch64_sve_SpSp_idxMatch_method2)
// SPSPFIXFUNCTION (svSpSpCustPerm,aarch64_sve_SpSp_custPerm)
SVSPSPMODEFUNCTION (svSpSpCustPerm,aarch64_sve_SpSp_custPerm,function_base)
SPSPFIXFUNCTION (svSpSpGetPred,aarch64_sve_SpSp_custGetPred)
SPSPFIXFUNCTION (svSpSpSingleSideSortInit,aarch64_sve_SpSp_singleSideSortInit)
SPSPFIXFUNCTION (svSpSpSingleSideSort,aarch64_sve_SpSp_singleSideSort)
SPSPFIXFUNCTION (svSpSpDiff,aarch64_sve_SpSp_diff)
SPSPFIXFUNCTION (svSpSpEncoding,aarch64_sve_SpSp_encoding)
SPSPFIXFUNCTION (svSpSpCustLastActElem,aarch64_sve_SpSp_custLastActElem)
SVSPSPMODEFUNCTION (svSpSpRingLoad,aarch64_sve_SpSp_ring_load,svSpSpMemoryBaseLoad)
SVSPSPMODEFUNCTION (svSpSpRingStore,aarch64_sve_SpSp_ring_store,svSpSpMemoryBaseStore)

#undef SPSPFIXFUNCTION

FUNCTION (svindex_compress, svindex_compress_impl,)
FUNCTION (svring_load, svring_load_impl,)
